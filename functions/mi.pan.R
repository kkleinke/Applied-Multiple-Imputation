mi.pan <- function(y, x, xcol, zcol, group, paniter = 1000, intercept=TRUE, m=5){
  # arguments:
  ############
  ...
  implist=vector(length=m, mode="list")
  for (i in 1:m){
    cat(i,"\n")
    s1 <- round(runif(1, 1, 10^7))
    imput <- pan(y1, subj, x, xcol, zcol, prior, seed = s1,
                 iter = paniter)
    res <- imput$y
    ...
    implist[[i]]<-data.frame(y=res,x,group=group)
  }
  return(implist)
}